...
[info,eeg] = edfread('participantID.edf'); % import data
Fs = info.frequency(1,1); % extract sampling frequency
ecg = []; % extract the ECG trace

% Cutting some data

% Rereference the data
[bipolar_montage,bipolar_labels,laplacian_montage,laplacian_labels] = rereference_scheme2(eeg);
...
amplitude_rejection = 100; % threshold of amplitude-based rejection
z_set = 5; % standard deviation set for RMS/LL
...
[bipolar_power_calcs] = power_params_v2(bipolar_segments_for_calcs,Fs);
[laplacian_power_calcs] = power_params_v2(laplacian_segments_for_calcs,Fs);
