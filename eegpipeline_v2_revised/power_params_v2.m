...
Outputs:
output is a matrix containing power spectral parameters for the data
contained in segmentsforcalc

ROWS. For all columns and depths, the rows can be interpreted as follows:
[Delta Power;Theta Power;Alpha Power;Beta Power;1/f slope]
...
for i = 1:size(segmentsforcalc,2)
    ...
    spectrograms{i} = pxx;
    %add keyboard control added here to save pxx to plot 1/f
    clear pxx
end
...
for i = 1:size(spectrograms,2)
    ...
    %normalization step?
    [bandz] = spectral1110_v2(freq,allspectra,F);
    %1/f
    logtransform = log10(allspectra);
    logtransform = logtransform(freq>=2 & freq<=20,:);
    f_for_slope = freq(freq>=2 & freq<=20,:);
